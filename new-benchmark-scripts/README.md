...
Every script reads this file. Columns:

```csv
filename,normalize,doc_count,dimensions,space_type
sift-128-euclidean.hdf5,false,1000000,128,l2
gist-960-euclidean.hdf5,false,1000000,960,l2
...
```
...
place them in your `--input-dir` / `--datasets-dir`, using the exact `filename` listed
in `datasets.csv`.

Some public download links are available in
[`../benchmarking/benchmarks.yml`](../benchmarking/benchmarks.yml):

| benchmarks.yml dataset | Download URL | Notes |
| --- | --- | --- |
| sift-128 | `https://ann-benchmarks.com/sift-128-euclidean.hdf5` | Filename matches `datasets.csv` directly. |
| gist-960 | `http://ann-benchmarks.com/gist-960-euclidean.hdf5` | Filename matches `datasets.csv` directly. |
| ms-marco-384 | `https://huggingface.co/datasets/navneet1v/datasets/resolve/main/ms_marco-384-1m.hdf5?download=true` | Downloaded name differs; rename to match your CSV row. |
| cohere-768-l2 | `https://huggingface.co/datasets/navneet1v/datasets/resolve/main/cohere-768-l2.hdf5?download=true` | |
| cohere-768-ip | `https://dbyiw3u3rf9yr.cloudfront.net/corpora/vectorsearch/cohere-wikipedia-22-12-en-embeddings/documents-1m.hdf5.bz2` | `bz2` — decompress before use. |
| cohere-10M-768-ip | `https://dbyiw3u3rf9yr.cloudfront.net/corpora/vectorsearch/cohere-wikipedia-22-12-en-embeddings/documents-10m.hdf5.bz2` | `bz2` — decompress before use. |
| open-ai-1536 | `https://huggingface.co/datasets/navneet1v/datasets/resolve/main/open-ai-1536-temp.hdf5?download=true` | |
| bigAnn-10M-128 | `https://huggingface.co/datasets/navneet1v/datasets/resolve/main/bigann-10M-with-gt.hdf5?download=true` | |

Example — download the two datasets whose names match `datasets.csv` as-is:
...
For a `bz2` dataset, download then decompress:
...
> column in `datasets.csv`. If the downloaded name differs (e.g. `ms_marco-384-1m.hdf5`
> vs a CSV row of `mini_msmarco.hdf5`), rename the file or edit the CSV so they agree.
>
> **Not every `datasets.csv` entry is in `benchmarks.yml`.** Only `sift-128-euclidean`
> and `gist-960-euclidean` map directly. Entries such as `coherev2-10m`, `mini_msmarco`,
> `mpnet_marco`, `marco_tasb`, `e5_embeddings`, `snowflake_embeddings`,
> `flickr-image-queries`, and `coherev2-dbpedia` are not covered by `benchmarks.yml` and
> must be obtained from your own source. Trim `datasets.csv` to just the rows you have
> HDF5 files for.
...
python remote_build_csv.py \
  ...
  -c datasets.csv \
  -g \
  -o build_results_graph_only.csv
```
...
| `-g`, `--graph-only` | Build graph only, do not store vectors in the index. |
| `-o`, `--output` | Output CSV (default `results.csv`). |

### Output columns

- Identity: `dataset`, `dimension`, `doc_count`, `data_type`, `space_type`, `graph_only`
