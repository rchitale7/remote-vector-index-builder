...
class HDF5DataSet(DataSet):
    ...
    def __init__(self, dataset_path: str, context: Context, custom_context=None):
        ...
        self.data = cast(
            h5py.Dataset, file[self._parse_context(context, custom_context)]
        )
        self.current = 0
...
class BigANNNeighborDataSet(DataSet):
    ...
    def read(self, chunk_size: int):
        ...
        v = [
            [int.from_bytes(self.file.read(4), "little") for _ in range(self.k)]
            for _ in range(end_i - self.current)
        ]
...
class BigANNVectorDataSet(DataSet):
    ...
    def read(self, chunk_size: int):
        ...
        v = np.asarray([self._read_vector() for _ in range(end_i - self.current)])
        self.current = end_i
        return v
    ...
    def _read_vector(self):
        return np.asarray([self.reader(self.file) for _ in range(self.dimension)])
